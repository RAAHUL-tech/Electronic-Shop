<%-- 
    Document   : mycart
    Created on : 07-Feb-2023, 7:27:07 pm
    Author     : rahul
--%>
    ...
    <header>
      <nav class="navbar navbar-default navbar-fixed-top navbar-inverse">
        ...
        <%
                   ...
                   PreparedStatement stmt=con.prepareStatement("select product_id,pname,pdes,amount,discount from cart where uname=?");
      ...
      <div class="container" id="product-cards">
       ...
       <div class="container outeritem">
           <% String id= rs.getString("product_id");
             if(id.contains("o-")){
           %>
           <img src="./Serv2?id=<%=id%>" width="200px" height="200px"/>
           <% }
             else{%>
                 <img src="./Serv1?id=<%=id%>" width="200px" height="200px"/>
             <%}%>
       <div class="container item">
           ...
           <%
               if(id.contains("o-")){
               char id1=id.charAt(2);
               %>
                <button type="submit" name="button" onclick="functio1(<%=id1%>)" value=<%=id1%>><span>Remove</span></button>
            <% }
             else{%>
               <button type="submit" name="button" onclick="func(<%=id%>)" value=<%=id%>><span>Remove</span></button>
             <%}%>
           </div>
           <hr width="700px">
       </div>
       ...
       <div class="container amount">
           ...
           else if(tot_amt>=50000)
           {
               gst=36;
           }
               %>
               <p>Taxes : <%=gst%>%</p>
               <% double total1=(gst*Math.pow(10, -2))*tot_amt;
                  
                   total=(int)total1+tot_amt;
               %>
               <p>Total :&#8377; <%=total%></p>
               <tag:if test="${true}">
                   <h3>Hurray you can avail free delivery service!!</h3>
               </tag:if>
                   <button type="submit" onclick="pay(<%=total%>)">Proceed to payment</button>
       </div>
         <% 
...
<script>
    function func(id)
    {
       // var id=document.getElementById("hide").value;
        ...
        xmlHttp.open( "POST","./removefromcart" ); 
    ...
    function functio1(myvar)
    {
       // var id=document.getElementById("hide").value;
       console.log(myvar);
       myvar="o-"+myvar;
        var xmlHttp1 = new XMLHttpRequest();
        xmlHttp1.open( "POST","./removefromcart2" ); 
        xmlHttp1.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlHttp1.send("id="+myvar);
        alert("Item removed from cart");
        setTimeout(function(){
        window.location.reload();
        }, 2000);
    ...
    function pay(total)
    {
       // var id=document.getElementById("hide").value;
        ...
        xmlHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xmlHttp.send("amt="+total);
        console.log(total);
        alert("Redirecting you to payment gateway");
        setTimeout(function(){
        window.location.assign("./pay?amt="+total);
        }, 1000);
    }
